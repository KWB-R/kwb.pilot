...
```{r a_import_upload, eval = eval_chunks}
...
move_nextcloud_files(paths_pilot_a, overwrite = TRUE)
```


```{r b_download, eval = eval_chunks}
...
```{r b_import_upload, eval = eval_chunks}
### Move all files for Pilot A`s cloud "raw" data directory to the "imported" 
### directory (in case that a file is already existing there: overwrite it)!
paths_pilot_b <- data.frame(raw = file.path(paths$a_cloud_raw, files_pilot_b),
                            imported = file.path(paths$a_cloud_imported, files_pilot_b)
                            )
```

```{r b_move, eval = eval_chunks}
move_nextcloud_files(paths_pilot_b, overwrite = TRUE)
