...
mbr4_data_tidy_list <- kwb.pilot::tidy_mbr4_data(mbr4_data = mbr4_data_raw) 

str(mbr4_data_tidy_list)
...
mbr4_data_tidy_list_10min <- mbr4_data_tidy_list %>%   
...
```{r export_data_wide_example, eval = TRUE}
mbr4_data_tidy_wide <- mbr4_data_tidy_list %>%  
  kwb.pilot::long_to_wide()

str(mbr4_data_tidy_wide)
```

For exporting all datasets to `.csv` run the following lines of code:

```{r export_data, eval = FALSE}

mbr4_data_tidy_list %>%  
  kwb.pilot::long_to_wide() %>% 
  kwb.pilot::export_data(export_dir = paths$export_dir)

mbr4_data_tidy_10min %>%  
  kwb.pilot::long_to_wide() %>% 
  kwb.pilot::export_data(export_dir = paths$export_dir)
