...
#' @param calc_list list with calculation operations to be carried out (default:
#'   list(deltaSAK  = "(1-SCAN_SAK_Ablauf/SCAN_SAK_Zulauf)*100", Ozoneintrag =
#'   "(C_O3_Zugas - C_O3_Abgas)*Q_Gas/Q_Ozonanlage"))
#' @param calc_list_name full names of parameters to be used for plotting for
#'   each calculation specified wit 'calc_list'. default: c('delta SAK',
#'   'Ozoneintrag')
...
#'   calculations defined in 'calc_list' (default: c("SCAN_SAK_Ablauf",
#'   "SCAN_SAK_Zulauf", "C_O3_Zugas", "C_O3_Abgas", "Q_Gas", "Q_Ozonanlage"))
#' @param config configuration object (list) from which the \code{calc_*}
#'   arguments are filled. Default: \code{get_calc_config("berlin_s")}
...
calculate_operational_parameters_berlin_s <- function(
  ...
  config = get_calc_config("berlin_s")
)
{
