...
)
{
  ...
  add_midnight <- function(x) paste(x, "00:00:00")  
  
  grp_list <- list(
    year = add_midnight("%Y-01-01"),
    month = add_midnight("%Y-%m-01"),
    day = add_midnight("%Y-%m-%d")
  )
  ...
  kwb.utils::catAndRun(text, dbg = dbg, expr = {
    df %>%
      dplyr::group_by(
        dplyr::across(.cols = dplyr::setdiff(names(df), "ParameterValue"))
      ) %>%
      dplyr::summarise(
        ParameterValue = eval(parse(
          text = paste0(fun, "(.data$ParameterValue)")
        )),
        .groups = "keep"
      ) %>%
      as.data.frame()
