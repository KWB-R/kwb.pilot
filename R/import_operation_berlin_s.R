...
read_wedeco_data <- function(
  raw_data_dir = shiny_file("berlin_s/data/operation"),
  raw_data_files = NULL,
...
)
{
  ...
  raw_list <- lapply(files_to_import, function(pathfile) {
      ...
      process_ids <- stringr::str_sub(names(ozone)[indices_names], 6L, 11L)
      
      names(ozone)[indices_units] <- process_ids
      
      ozone <- ozone[, c(1L, indices_units), with = FALSE]
      
      names(ozone)[1L] <- "DateTime"
...
)
{
  ...
  remove_duplicates(df = df, col_names = c(
